...
trans2iso(JNIEnv *env, int haveutf, jstring enc, jstring src, transstr *dest) {
	...
	if (enc) {
		bytes = (*env)->CallObjectMethod(env, src, M_java_lang_String_getBytes2,
				enc);
	} else {
...
static jstring trans2utf(JNIEnv *env, int haveutf, jstring enc, const char *src,
		transstr *dest) {
	jbyteArray bytes = 0;
	int len;
...
JNIEXPORT void JNICALL
Java_SQLite3_Database__1open4(JNIEnv *env, jobject obj, jstring file, jint mode,
...
Java_SQLite3_Database__1exec__Ljava_lang_String_2LSQLite3_Callback_2(
	...
	if (h) {
		if (h->sqlite) {
			...
			rc = sqlite3_exec((sqlite3 *) h->sqlite, sqlstr.result, callback, h,
					&err);
...
JNIEXPORT void JNICALL
Java_SQLite3_Database__1create_1function(JNIEnv *env, jobject obj, jstring name,
		jint nargs, jobject fi) {
	...
	if (v && v->vm && v->h) {
		...
		if (ret == SQLITE_DONE && v->hh.row1) {
			...
			if (ret != SQLITE_NOMEM) {
				...
				for (i = 0; i < ncol; i++) {
					cols[i] = sqlite3_column_name((sqlite3_stmt *) v->vm, i);
					if (sqlite3_column_type((sqlite3_stmt *) v->vm,
							i) == SQLITE_BLOB) {
						unsigned char *src =
...
JNIEXPORT void JNICALL
Java_SQLite3_Stmt_bind__ILjava_lang_String_2(JNIEnv *env, jobject obj, jint pos,
		jstring val) {
	hvm *v = gethstmt(env, obj);
...
JNIEXPORT jint JNICALL
Java_SQLite3_Database__1status(JNIEnv *env, jclass cls, jint op, jintArray info,
		jboolean flag) {
...
Java_SQLite3_Database_internal_1init(JNIEnv *env, jclass cls) {
	...
	M_java_lang_String_initBytes = (*env)->GetMethodID(env, C_java_lang_String,
			"<init>", "([B)V");
	M_java_lang_String_initBytes2 = (*env)->GetMethodID(env, C_java_lang_String,
