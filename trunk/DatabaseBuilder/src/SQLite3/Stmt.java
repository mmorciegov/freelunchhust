...
/**
 * Class to represent compiled SQLite3 statement. Note, that all native methods
 * of this class are not synchronized, i.e. it is up to the caller to ensure
 * that only one thread is in these methods at any one time.
 */
...
public class Stmt {
	...
	/**
	 * Perform one step of compiled SQLite3 statement. Example:<BR>
	 * 
	 * <PRE>
	 *   ...
	 *   try {
	...
	public Object column(int col) throws SQLite3.Exception {
		...
		case Constants.SQLITE_FLOAT:
			return Double.valueOf(column_double(col));
