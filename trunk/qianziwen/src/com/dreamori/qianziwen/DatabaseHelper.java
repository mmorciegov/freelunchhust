...
public class DatabaseHelper {
	...
	public static void closeDatabase()
	{
		try {
			...
			DreamoriLog.LogQianZiWen("Close Data Base success.");
    ...
    {    	
        ...
        if (!targetFile.exists() || forceReplace)        
    	{
			try {
				...
				DreamoriLog.LogQianZiWen("copyBigDataBase");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	private final int TextSpellCount = 16;
	private final int TextSplitCount = 4;
	...
	public int GetTextContent( int inputId, String[] textContent )
	{
    	...
    	if( outputCount % TextSplitCount != 0 )
    	{
    		outputCount = ( (outputCount + 1)/TextSplitCount ) * TextSplitCount ;
    	}
    	
    	for( int k = 0; k < outputCount/TextSplitCount; k ++ )
    	{
    		for( int s = 0; s< TextSplitCount; s++ )
    		{
    			textContent[ TextSplitCount*2*k + s ] = (spellStrings[ TextSplitCount*k  + s ]).trim();
    			textContent[ TextSplitCount*(2*k+1) + s] = textResult.substring(TextSplitCount*k + s, TextSplitCount*k+ s + 1);
    		}
    	}
