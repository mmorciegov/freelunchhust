...
import com.dreamori.dizigui.ParameterObject.MusicInfo;
...
public class PlayerService extends Service {
	...
	public final static String ACTION_UPDATE_PLAYING = "com.dreamori.dizigui.update_playing";
	...
	void StartTimerForMeidaPlayer()
	{
		...
		getDatabaseHelper().GetMusicInfo(DiZiGui.m_currentImageIndex , musicInfo);
			...
			public void run() {
				...
				else if(CurrentPlayMode.equals(getString(R.string.mode_all)))
				{
					...
					{						
						...
						if( currPos >= musicInfo.stopTime  ||  currPos < musicInfo.startTime )
						{
							if(DiZiGui.m_currentImageIndex == Const.m_maxImageIndex)
							{
								stopSelf();
				...
				else if(CurrentPlayMode.equals(getString(R.string.mode_all_cycle)))
				{
					...
					{						
						...
						if( currPos >= musicInfo.stopTime  ||  currPos < musicInfo.startTime )
						{
							...
							if( DiZiGui.m_currentImageIndex > Const.m_maxImageIndex )
							{
								DiZiGui.m_currentImageIndex = Const.m_minImageIndex;
								StartPlay();
							}
							UpdatePage();
							getDatabaseHelper().GetMusicInfo(DiZiGui.m_currentImageIndex , musicInfo);
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		Intent intent1 = new Intent(this, DiZiGui.class);
	...
	public void onDestroy() {
		super.onDestroy();
		DreamoriLog.LogDiZiGui("PlayerService onDestroy");	
	...
	private void UpdatePage()
	{
        ...
        intent.setAction(DiZiGui.ACTION_UPDATE_PAGE);
	...
	{
		Intent intent = new Intent();  
        intent.setAction(DiZiGui.ACTION_UPDATE_PLAY_STATE);
        sendBroadcast(intent);
	}
