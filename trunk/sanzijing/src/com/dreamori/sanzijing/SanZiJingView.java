...
public class SanZiJingView extends RelativeLayout {

	public final static int TEXTVIEW_COUNT = 48;
	...
	public SanZiJingView(Context context) {
		this(context, null);
	}
	
	public SanZiJingView(Context context, AttributeSet attrs) {
		super(context, attrs);
		
		LayoutInflater.from(context).inflate(R.layout.sanzijing_view, this, true);  
	...
	public void updateContent(String[] textContent, String contentIndexString, int outputSpellCount)
	{
		...
		for( int i = 0; i < outputSpellCount*2; i++ )
		{
			...
			if((i + 1) % 6 == 0)
			{
				View v = findViewById(ResourceManager.GetViewId(mContext, "pun"+(i + 1)/6));
				if(v != null)
				{
		...
		if( outputSpellCount*2 < TEXTVIEW_COUNT )
		{
			for( int j = outputSpellCount*2; j < TEXTVIEW_COUNT; j ++ )
			{
				...
				if((j + 1) % 6 == 0)
				{
					View v = findViewById(ResourceManager.GetViewId(mContext, "pun"+(j + 1)/6));
					if(v != null)
					{
